...
type GuildNavigationRow =
	...
	| (NavigationRow & {readonly kind: 'member-registration-invite'})
...
function createGuildNavigationRows({
	...
	rows.push({
		kind: 'member-registration-invite',
		key: 'member-registration-invite',
		focusable: true,
		focusTargetIdentity: 'member-registration-invite',
	});
...
function hasGapAfterGuildNavigationRow(row: GuildNavigationRow, nextRow: GuildNavigationRow | null): boolean {
	switch (row.kind) {
		...
		case 'discovery':
		case 'add-guild':
		case 'member-registration-invite':
		case 'download':
			return true;
		case 'favorites':
			if (nextRow == null) {
...
const GuildList = observer(() => {
	...
	const renderGuildNavigationRow = (row: GuildNavigationRow): React.ReactNode => {
		switch (row.kind) {
			...
			case 'member-registration-invite':
				return (
					<MemberRegistrationInviteButton data-flx="app.guilds-layout.render-guild-navigation-row.member-registration-invite-button" />
				);
			case 'download':
