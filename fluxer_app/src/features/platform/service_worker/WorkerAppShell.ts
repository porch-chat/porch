...
import {createNavigationNetworkRequest} from '@app/features/platform/service_worker/WorkerNavigationPolicy';
...
export async function fetchAppShellNavigation(runtime: AppShellRuntime, request: Request): Promise<Response> {
	...
	const network = runtime.fetch(createNavigationNetworkRequest(request)).then(async (response) => {
		await storeAppShell(runtime, response);
		return response;
	});
