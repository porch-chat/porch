...
class MediaEngineFacade extends Store {
	...
	async getDeviceScreenShareSourceFormats(
	...
	): Promise<Array<{width: number; height: number; frameRate: number; pixelFormat: string}>> {
		...
		try {
			...
			const width = capabilities.width?.max ?? settings.width;
			const height = capabilities.height?.max ?? settings.height;
			const frameRate = Math.min(60, capabilities.frameRate?.max ?? settings.frameRate ?? 30);
			if (!width || !height) return [];
			return [{width, height, frameRate, pixelFormat: 'browser'}];
		} finally {
